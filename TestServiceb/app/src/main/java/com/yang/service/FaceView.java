...
public class FaceView extends android.support.v7.widget.AppCompatImageView {
    ...
    private int width;
    private float dis;
    static List<Float> eyeDistanceList = new ArrayList<Float>();    //eyeDistance数组中存放获取的眼部距离
    ...
    private void calculateFaceArea(){
        ...
        for(int i = 0; i < faces.length; i++){
            ...
            if(face != null){
                ...
                dis=eyesDistance;
                eyeDistanceList.add(dis);
                System.out.println("垂直时两眼之间的距离列表"+eyeDistanceList);
                ListAngle.setFaceAngle(eyeDistanceList);
                //计算面部角度
                //用户的视线是手机屏幕垂直时的眼部距离
                float initEyes = eyeDistanceList.get(0);
                float faceAngle= (initEyes-dis)/5;
                Data.setFaceangel(faceAngle);
                System.out.println("垂直时两眼之间的距离"+dis);
                //计算人脸的区域
