...
public class StartService extends Service {
	...
	int i = 0;
	public void detectFace(Bitmap srcImg){
    ...
    public List<Double> test(Bitmap srcImg,float phoneTiltAngle){
		if (averagelEyesDis >= 0) {
			detectFace(srcImg);
			//final double faceAngle = (dis - averagelEyesDis) / 0.1;
			final double faceAngle = Data.getFaceangel();
			neckAngle.add(faceAngle + (90-phoneTiltAngle));
			System.out.println(faceAngle+"----");
			//neckAngle.add(faceAngle);
			if(faceAngle+phoneTiltAngle > 60){
				/*long [] pattern = {100,400,100,400}; // 停止 开启 停止 开启
				vibrator.vibrate(pattern,-1);*/
			}
